...
export type DataPublishedDependencies = {
  readRemotePackManifest: () => Promise<PackManifest>;
  revalidateTag: typeof revalidateTag;
  revalidatePath: typeof revalidateReleasePath;
};

const defaultDependencies: DataPublishedDependencies = {
  readRemotePackManifest,
  revalidateTag,
  revalidatePath: revalidateReleasePath,
};

export async function handleDataPublished(
  request: Request,
  dependencies: DataPublishedDependencies = defaultDependencies,
  ...
  const manifest = await dependencies.readRemotePackManifest();
  const servedReleaseId = manifest.pack_id;
  const matches = servedReleaseId === body.release_id;
  if (matches) {
    dependencies.revalidateTag(PACK_MANIFEST_CACHE_TAG, { expire: 0 });
    for (const target of REVALIDATED_TARGETS) dependencies.revalidatePath(target.path, target.type);
  }
  return NextResponse.json(
    {
      revalidated: matches,
