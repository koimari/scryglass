...
The v3 harness consumes the strong `scryglass:resolved-rating-source:v1` contract from rating producer commit `9420a1cb`. Its canonical receipt binds the source hash, exact nullable rating values, rating timestamp, group availability, exact roster hash, and the equal-time batch receipt and policy. The roster payload also binds the game ID, map timestamp, source identity, team IDs, player IDs, sides, roles, and champions. The consumer recomputes the roster and batch hashes. Legacy receipts with the same schema label fail. A missing or unavailable team or player group stays missing and cannot update its residual history. The current consumed matrix predates this producer contract, so its rating and momentum evidence remains unavailable.
