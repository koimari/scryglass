...
| REL-008 | High | A superseded release can be restored after its objects changed or disappeared. | Supabase restore RPC and publisher restore path. | Data platform | Reuse the activation inventory checks and hash every object before restore. | Missing, changed, Draft-bearing, and valid prior release restore cases. | In progress. |
...
| CI-002 | High | Main can merge without browser, Supabase, security, or second Python-suite gates. | GitHub Actions and ruleset `20711858`. | CI and repository owner | Add stable jobs, then require them on protected `main`. | Clean-checkout workflow run with every job green. | Workflow in progress; ruleset update pending. |
| API-001 | High | Serverless instances can each reset a process-local chat request budget. | `/api/chat/*` and `/api/data-published`. | Web operations | Keep local budgets and publish the reviewed Vercel Firewall rules. | Boundary, burst, method, body, and multi-request probes. | Local limits closed; deployment-wide rules remain log-only. |
