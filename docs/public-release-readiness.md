...
| REL-010 | Critical | The active release can serve bytes whose stored digest is not the digest claimed by the release receipt. | Production `v2026.08.14.123517`, `features/match_records_2025_q2.json`; the 19,047,051-byte response carries the empty SHA-256 digest. | Publisher and operations | Keep the active release on hold. Create a fresh Storage-only release with byte hashing before activation, then verify every manifest asset from the deployed URL. | Equal-size content mutation and a full active-release asset hash crosscheck fail closed. | Open. The current release must not be used as the final receipt. |
...
| UI-003 | Medium | Important routes lack consistent loading, error, offline, empty, and missing states. | App Router pages and profiles. | Web | Add route-level boundaries and explicit states. | Desktop, tablet, 390 px, keyboard, reduced-motion, offline, empty, and error checks. | Closed in branch. Browser coverage passes. Preview and production checks remain open. |
