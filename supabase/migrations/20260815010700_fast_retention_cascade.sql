-- Keep trusted release-owner cascades out of the per-row lock path. The
-- parent release trigger has already checked the release state before the
-- foreign-key cascade reaches the sealed query tables.
