-- Add the per-map profile statistics asset that backs the profile Stats tab.
-- Display-only and optional: it never takes part in the ratings release gate,
-- so it is added to the path allowlist without joining `required_assets` in
-- assert_scryglass_public_release_integrity.
