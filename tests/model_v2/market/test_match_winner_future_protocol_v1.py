...
def test_protocol_is_empty_two_stage_and_non_authorizing() -> None:
    payload = _build()
...
def test_protocol_binds_exact_registered_prerequisites() -> None:
    payload = _build()
...
def test_rehashed_protocol_cannot_relax_quote_timing() -> None:
    ...
    with pytest.raises(MatchWinnerFutureProtocolError, match="quote capture contract"):
        validate_match_winner_future_protocol_v1(payload, root=ROOT)


def test_rehashed_protocol_cannot_grant_authority() -> None:
    payload = deepcopy(_build())
...
def test_rehashed_protocol_cannot_hide_source_drift() -> None:
    payload = deepcopy(_build())
...
def test_lock_must_precede_existing_future_boundary() -> None:
...
def test_writer_refuses_to_replace_protocol(tmp_path: Path) -> None:
    payload = _build()
...
def test_registered_protocol_replays_exact_bytes_and_contracts() -> None:
    payload = validate_registered_match_winner_future_protocol_v1(root=ROOT)
    ...
    raw = (ROOT / "data/lol/v2/evaluation/match-winner-market-v1/future-protocol-v1.json").read_bytes()
